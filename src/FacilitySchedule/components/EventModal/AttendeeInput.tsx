...
export function AttendeeInput({
    ...
    const handleKeyDown = (e: React.KeyboardEvent) => {
        ...
        if (open && filteredPersonnel.length > 0) {
            if (e.key === 'ArrowDown') {
                e.preventDefault();
                setActiveIndex(prev => (prev === -1 ? 0 : (prev + 1) % filteredPersonnel.length));
                return;
            } else if (e.key === 'ArrowUp') {
                e.preventDefault();
                setActiveIndex(prev => {
                    if (prev === -1) return filteredPersonnel.length - 1;
                    return (prev - 1 + filteredPersonnel.length) % filteredPersonnel.length;
                });
                return;
            }
        }
    ...
    return (
        ...
        >
            {value.map((attendee, i) => (
                <div key={i} className="flex items-center gap-1 bg-secondary text-secondary-foreground px-2 py-1 rounded text-sm group">

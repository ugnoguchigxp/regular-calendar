...
export function MonthView({
  ...
  const { filteredResources, filteredEvents } = useMemo(() => {
    ...
    return {
      filteredResources: resources.filter((pos) => pos.groupId === selectedGroupId),
  ...
  const densityMap = useMemo(() => {
    ...
    for (const day of calendarDays) {
      ...
      const density = maxSlotsPerDay > 0 ? (dayEvents.length / maxSlotsPerDay) * 100 : 0;
  ...
  const monthlyUtilization = useMemo(() => {
    const monthDays = calendarDays.filter((day) => day.getMonth() === month.getMonth());
    const totalDensity = monthDays.reduce((sum, day) => {
      const dayKey = getDayKey(day);
      return sum + (densityMap.get(dayKey) || 0);
    }, 0);

    return monthDays.length > 0 ? Math.round(totalDensity / monthDays.length) : 0;
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="w-full p-4 overflow-y-auto">
        <div className="w-full">
          ...
          {/* Days */}
          <div className="grid gap-1" style={{ gridTemplateRows: `repeat(${weeks.length}, 1fr)` }}>
            {weeks.map((week) => (
              <div key={week[0]?.toISOString() ?? 'empty-week'} className="grid grid-cols-7 gap-1">
                {week.map((day) => {
                  const dayKey = day.toISOString().split('T')[0] || '';
                  const density = densityMap.get(dayKey) || 0;
                  const isCurrentMonth = day.getMonth() === month.getMonth();
                  const isClosedDay = settings.closedDays.includes(day.getDay());

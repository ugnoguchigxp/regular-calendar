...
describe('useResourceAvailability', () => {
  ...
  it('filters available resources', () => {
    ...
    expect(result.current.availableResources.every((r) => r.id !== 'resource-1')).toBe(true);
  });
  ...
  it('returns display name for resource', () => {
    ...
    expect(result.current.getDisplayName('resource-1')).toBe('Resource 1 (Group 1)');
    expect(result.current.getDisplayName('resource-2')).toBe('Resource 2 (Group 1)');
    expect(result.current.getDisplayName('resource-3')).toBe('Resource 3 (Group 2)');
  });

    ...
    const processAttendeesForSubmit = useMemo(() => {
        return (attendeeStr: string) => {
            ...
            if (!isEditMode && currentUserId && personnel) {
                const isMeIncluded = finalAttendeeList.some((a: AttendeeInfo) => a.personnelId === currentUserId);
                if (!isMeIncluded) {
            ...
            return {
                finalAttendees: finalAttendeeList.length > 0 ? JSON.stringify(finalAttendeeList) : '[]',
                shouldDelete: false,
            };
        };

...
describe('useScheduleView', () => {
  ...
  it('goes to today when goToToday is called', () => {
    ...
    expect(result.current.currentDate.getFullYear()).toBeGreaterThanOrEqual(2025);
  });
  ...
  describe('persistence', () => {
    it('loads view mode from storage when persistence is enabled', () => {
      ...
      expect(defaultStorage.getItem).toHaveBeenCalledWith('facility-schedule-view');
      expect(result.current.viewMode).toBe('week');
    });
    ...
    it('saves view mode to storage when it changes', () => {
      ...
      expect(defaultStorage.setItem).toHaveBeenCalledWith('facility-schedule-view', 'week');
    });

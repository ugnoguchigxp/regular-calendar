...
import { useAttendeeManagement, parseAttendees, stringifyAttendees } from './useAttendeeManagement';
...
describe('parseAttendees', () => {
  ...
  it('parses JSON array of attendees', () => {
    const result = parseAttendees(
      JSON.stringify([
        { name: 'John Doe', email: 'john@example.com', personnelId: '1', type: 'personnel' },
...
describe('stringifyAttendees', () => {
  ...
  it('converts attendees to JSON string', () => {
    ...
    expect(result).toBe('[{"name":"John Doe","type":"external"},{"name":"Jane Smith","type":"external"}]');
  });
});
...
describe('useAttendeeManagement', () => {
  ...
  describe('processAttendeesForSubmit', () => {
    ...
    it('returns shouldDelete false in edit mode when attendees exist', () => {
      ...
      const output = result.current.processAttendeesForSubmit(JSON.stringify(attendees));
    ...
    it('adds current user to attendees in create mode if not already included', () => {
      ...
      expect(parsed.some((a: any) => a.personnelId === 'person-1')).toBe(true);
    });
    ...
    it('does not add current user if already included', () => {
      ...
      const parsed = JSON.parse(output.finalAttendees);
      expect(parsed).toHaveLength(2);
      expect(parsed.filter((a: any) => a.personnelId === 'person-1')).toHaveLength(1);
    });

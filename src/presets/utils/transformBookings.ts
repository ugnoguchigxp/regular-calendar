...
import { getPersonnelColor } from '../../PersonnelPanel';
import { formatEventTitleWithAttendees, resolveEventOwnerId } from './eventHelpers';
...
export interface BookingData {
    ...
    isAllDay: boolean;
    attendee: string;
    extendedProps?: string | Record<string, any>;
}
 ...
 */
function parseExtendedProps(extendedProps: string | Record<string, any> | undefined): Record<string, any> {
    if (!extendedProps) return {};
    if (typeof extendedProps === 'string') {
        try {
            return JSON.parse(extendedProps);
        } catch {
            console.warn('[transformBookings] Failed to parse extendedProps', extendedProps);
            return {};
        }
    }
...
export function transformBookingsToEvents(
    ...
    return availability.flatMap(res =>
        res.bookings.map((book): ScheduleEvent => {
            ...
            const displayTitle = formatEventTitleWithAttendees(book.title, book.attendee);

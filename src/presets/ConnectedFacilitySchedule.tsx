...
import type { ScheduleEvent, ViewMode } from '../FacilitySchedule/FacilitySchedule.schema';
import { transformBookingsToEvents, type ResourceAvailabilityResponse } from './utils/transformBookings';
...
export function ConnectedFacilitySchedule({ settings, currentUserId }: ConnectedFacilityScheduleProps) {
    const {
        resources,
        groups,
    ...
    const fetchData = useCallback(async () => {
        setIsFetching(true);
        try {
            const availability = await fetchResourceAvailability(currentDate, viewMode) as unknown as ResourceAvailabilityResponse[];

            // Use extracted utility function for transformation
            const flattenedEvents = transformBookingsToEvents(availability, personnel);

            console.log(`[ConnectedFacilitySchedule] Loaded ${flattenedEvents.length} events for ${viewMode} view`);

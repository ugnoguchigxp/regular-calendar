...
vi.mock('./utils/transformBookings', () => ({
    transformBookingsToEvents: (...args: any[]) => mockTransform(...args),
}));

vi.mock('../FacilitySchedule/FacilitySchedule', () => ({
    FacilitySchedule: ({ onEventCreate, onEventUpdate, onEventDelete }: any) => (
        <div>
            <button type="button" onClick={() => onEventCreate({ title: 'New' })}>
                Create
            </button>
            <button type="button" onClick={() => onEventUpdate('e1', { title: 'Update' })}>
                Update
            </button>
...
describe('ConnectedFacilitySchedule', () => {
        ...
        await act(async () => {
            render(
                <ConnectedFacilitySchedule
                    settings={{ weekStartsOn: 1, businessHoursStart: '08:00', businessHoursEnd: '18:00' }}
                />
            );
    ...
    it('fetches availability and calls create/update/delete', async () => {
        ...
        mockTransform.mockReturnValue([{ id: 'e1', title: 'Event', resourceId: 'r1' }]);
        ...
        mockContext.mockReturnValue({
            ...
            personnel: [{ id: 'p1', name: 'Owner', priority: 0, email: 'a@a.com', department: 'X' }],
        });

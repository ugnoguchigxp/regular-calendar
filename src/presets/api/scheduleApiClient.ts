 ...
 */
function parseEventDates(event: any): ScheduleEvent {
    return {
        ...event,
...
export function createScheduleApiClient(baseUrl: string): ScheduleApiClient {
    ...
    return {
        ...
        async createEvent(data: EventData): Promise<ScheduleEvent> {
            const event = await fetchJson<any>(getUrl('/events'), {
                method: 'POST',
        ...
        async updateEvent(id: string, data: Partial<EventData>): Promise<ScheduleEvent> {
            const event = await fetchJson<any>(getUrl(`/events/${id}`), {
                method: 'PUT',
                body: JSON.stringify(data),
            });
            return parseEventDates(event);
        ...
        async updateGroup(id: string, data: Partial<ResourceGroup>): Promise<ResourceGroup> {
        ...
        async updatePersonnelPriority(id: string, priority: number): Promise<Personnel> {
        ...
        async getResourceAvailability(params: ResourceAvailabilityParams): Promise<ResourceAvailabilityResponse[]> {
            const { date, view } = params;
            const url = getUrl(`/resource-availability?date=${date.toISOString()}&view=${view}`);
            const data = await fetchJson<{ resources: ResourceAvailabilityResponse[] }>(url);

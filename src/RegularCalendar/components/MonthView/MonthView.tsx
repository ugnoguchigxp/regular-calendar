    ...
    return (
        <div className="h-full flex flex-col bg-background text-foreground">
            ...
            <div className="grid grid-cols-7 border-b border-border bg-muted/40">
                {displayDayNames.map((day, i) => (
                    <div key={i} className="p-3 text-center text-sm font-medium text-muted-foreground border-r border-border last:border-r-0">
                        {day}
                    </div>
                ))}
            </div>
            ...
            {/* Grid */}
            <div className="flex-1 grid gap-0" style={{ gridTemplateRows: `repeat(${calendarGrid.length}, 1fr)` }}>
                {calendarGrid.map((week, weekIndex) => (
                    <div key={weekIndex} className="grid grid-cols-7 border-b border-border last:border-b-0 h-full">
                        {week.map((date) => {
                                ...
                                >
                                    ...
                                    <div className="space-y-1 overflow-hidden flex-1">
                                        {dayEvents.slice(0, 3).map((event) => (
                                            <MonthEventItem key={event.id} event={event} onClick={onEventClick} />
                                        ))}
                                        {dayEvents.length > 3 && (

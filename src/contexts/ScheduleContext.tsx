...
export function ScheduleProvider({
	...
	useEffect(() => {
		const loadData = async () => {
			if (!apiClient) {
				console.error(
					"ScheduleProvider: apiClient is undefined. Cannot load data.",
				);
				setLoading(false);
				return;
			}
	...
	const createEvent = useCallback(
		async (data: EventData) => {
			if (!apiClient) {
				console.error(
					"ScheduleProvider: apiClient is undefined. Cannot create event.",
				);
				return;
			}
			try {
	...
	const updateEvent = useCallback(
		async (id: string, data: Partial<EventData>) => {
			if (!apiClient) {
				console.error(
					"ScheduleProvider: apiClient is undefined. Cannot update event.",
				);
				return;
			}
			try {
	...
	const deleteEvent = useCallback(
		async (id: string) => {
			if (!apiClient) {
				console.error(
					"ScheduleProvider: apiClient is undefined. Cannot delete event.",
				);
				return;
			}
			try {
	...
	const fetchPersonnelEvents = useCallback(
		async (personnelIds: string[], append = false) => {
			if (!apiClient) {
				console.error(
					"ScheduleProvider: apiClient is undefined. Cannot fetch personnel events.",
				);
				return;
			}

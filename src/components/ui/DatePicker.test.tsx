...
vi.mock('./Calendar', () => ({
    Calendar: ({ onSelect, mode }: { onSelect?: (value: any) => void; mode?: string }) => (
        <button
            type="button"
            onClick={() =>
                onSelect?.(
                    mode === 'range'
                        ? { from: new Date('2024-02-01T00:00:00Z'), to: new Date('2024-02-03T00:00:00Z') }
...
describe('DatePicker', () => {
    ...
    it('renders formatted single date', () => {
        ...
        expect(screen.getByRole('button', { name: /2024年01月15日/i })).toBeInTheDocument();
    });
    ...
    it('renders formatted range date and calls onRangeChange', async () => {
        ...
        expect(screen.getByRole('button', { name: /2024年02月01日/i })).toBeInTheDocument();

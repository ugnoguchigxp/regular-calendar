...
import { useSettings as useLibrarySettings, type AppSettings } from 'regular-calendar';
...
describe('useSettings', () => {
  ...
  it('onLanguageChangeコールバックがi18nの言語を変更すること', () => {
    ...
    const onLanguageChange = (useLibrarySettings as any).mock.calls[0]?.[0]?.onLanguageChange;
  ...
  it('updateSettingsが正しく渡されること', () => {
    ...
    expect(mockLibrarySettings.updateSettings).toHaveBeenCalledWith({ theme: 'dark' });
  });

...
import { useScheduleContext as useMockScheduleApi, type AppSettings } from 'regular-calendar';
...
vi.mock('regular-calendar', () => ({
  ConnectedCalendar: vi.fn(() => <div data-testid="connected-calendar">Calendar</div>),
  ConnectedFacilitySchedule: vi.fn(() => <div data-testid="facility-schedule">Facility Schedule</div>),
  ScheduleProvider: ({ children }: { children: React.ReactNode }) => <div>{children}</div>,
  SettingsModal: vi.fn(({ isOpen, onClose, onUpdateSettings, onResetSettings }: any) =>
    isOpen ? (
      <div data-testid="settings-modal">
        <button onClick={onClose}>Close</button>
        <button onClick={() => onUpdateSettings({ theme: 'dark' as const })}>Update</button>
  ...
  PersonnelPanel: vi.fn(() => <div data-testid="personnel-panel">Personnel Panel</div>),
  ResizablePanel: vi.fn(({ children }: any) => <div data-testid="resizable-panel">{children}</div>),
...
describe('App', () => {
  ...
  it('デフォルトで「通常カレンダー」タブが選択されていること', () => {
    ...
    const regularButton = screen.getByRole('button', { name: /regular calendar/i });
  ...
  it('「施設スケジュール」タブをクリックすると施設スケジュールが表示されること', async () => {
    ...
    const facilityButton = screen.getByRole('button', { name: /facility schedule/i });
  ...
  it('「施設構造」ボタンをクリックすると施設構造モーダルが表示されること', async () => {
    ...
    const facilityStructureButton = screen.getByRole('button', { name: /facility structure/i });
  ...
  it('施設構造モーダルの閉じるボタンをクリックするとモーダルが閉じること', async () => {
    ...
    expect(screen.queryByTestId('facility-settings-modal')).not.toBeInTheDocument();
  });

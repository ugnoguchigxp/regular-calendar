...
import type { Personnel } from "regular-calendar";
	...
	describe("transformBookingsToEvents", () => {
		...
		it("should parse dates correctly", () => {
			...
			expect(result[0].startDate.getTime()).toBe(
				new Date("2025-01-01T10:00:00").getTime(),
			);
		});
		...
		it("should handle all-day events", () => {
			...
			const result = transformBookingsToEvents(
				availabilityWithAllDay,
				personnel,
			);
					...
					bookings: [
						{
							...
							isAllDay: false,
							attendee: "[]",
							extendedProps: { ownerId: "p1", custom: "value" },
						},
					],
				},
			];
		...
		it("should handle invalid extendedProps JSON", () => {
			...
			const result = transformBookingsToEvents(
				availabilityWithInvalidProps,
				personnel,
			);

			expect(result[0].extendedProps).toHaveProperty(
				"originalTitle",
				"Meeting",
			);
